...
import com.bezkoder.spring.security.jwt.payload.request.SectionDto;
import com.bezkoder.spring.security.jwt.payload.response.MessageResponse;
import com.bezkoder.spring.security.jwt.service.SectionService;
...
public class SectionController {
    ...
    @GetMapping("/enable/type/{type}")
    public ResponseEntity<List<Section>> getSectionsByType(@PathVariable Integer type) {
        List<Section> sectionList = sectionService.getSectionsByType(type);
        List<Section> filteredSectionList = sectionList
                .stream()
                .filter(section -> section.getStatus() == 1)
                .collect(Collectors.toList());
